...
def to_square(file_in):
    ...
    if(img_size[0]>img_size[1]):
        region = image.crop(((img_size[0]-img_size[1])/2, 0, (img_size[0]-img_size[1])/2+img_size[1], img_size[1]))
    else:
        region = image.crop((0, (img_size[1]-img_size[0])/2, img_size[0], (img_size[1]-img_size[0])/2+img_size[0]))
...
def paddle_hub(in_path,out_path,i):
    ...
    #裁剪图像
    results = huseg.segmentation(output_dir=out_path,data={'image': [in_address]},visualization=True) # 抠图
